...
<label class="{{ sampaui_classes(['inline-flex cursor-pointer items-center gap-3', 'cursor-not-allowed opacity-50' => $disabled]) }}" for="{{ $id }}">
    ...
    >
    <span class="{{ sampaui_classes(['inline-flex h-7 w-12 shrink-0 items-center rounded-full border border-secondary/40 bg-light p-1 transition peer-checked:[&>span]:translate-x-5 peer-focus:outline-none peer-focus:ring-2', sampaui_toggle_color_classes($color)]) }}">
